// WEB Provider: loads public‑domain World English Bible JSON per book from the public folder.
// Path: /bible/web/<Book>.json with same supported formats as KJV provider.
...
const BOOK_FILENAMES = {
  ...
  // Optional Apocrypha if supplied
  'Wisdom of Solomon': 'Wisdom of Solomon.json', 'Sirach': 'Sirach.json', 'Baruch': 'Baruch.json', 'Letter of Jeremiah': 'Letter of Jeremiah.json'
};
...
export async function loadBookWEB(bookName) {
  const file = BOOK_FILENAMES[bookName];
  if (!file) return null;
  if (cache.has(bookName)) return cache.get(bookName);
  try {
    const res = await fetch(publicUrl(`/bible/web/${file}`));
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const json = await res.json();
    cache.set(bookName, json);
    return json;
  } catch (e) {
    console.warn(`[WEB] Could not load book JSON for ${bookName}:`, e.message);
...
export async function getWEBVerses(book, chapter, vStart, vEnd) {
  const data = await loadBookWEB(book);
  ...
  for (let v = start; v <= end; v++) {
    ...
    if (text) out.push({ reference: `${book} ${chapter}:${v}`, text, translation: 'WEB' });
  }
  return out;
}

export default { loadBookWEB, getWEBVerses };
