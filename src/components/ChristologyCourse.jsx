...
import {
  BookOpen, ChevronRight, CheckCircle, ArrowLeft, Book, Scroll, Crown
} from 'lucide-react';
...
const ChristologyCourse = ({ onComplete, onCancel, userId, userData, setUserData }) => {
  ...
  const [completedLessons, setCompletedLessons] = useState(() => {
    return userData?.christologyProgress?.completedLessons || [];
  });

  // Hydrate from userData when it changes (handles late Firebase load)
  useEffect(() => {
    if (userData?.christologyProgress?.completedLessons) {
      setCompletedLessons(userData.christologyProgress.completedLessons);
    }
  }, [userData?.christologyProgress]);

  // Persist progress to localStorage + Firebase
  useEffect(() => {
    const progressPayload = { completedLessons };
    localStorage.setItem('christologyProgress', JSON.stringify(progressPayload));

    if (setUserData) {
      setUserData(prev => ({ ...prev, christologyProgress: progressPayload }));
    }

    if (userId) {
      updateUserProgress(userId, { christologyProgress: progressPayload }).catch(err =>
        console.error('Error saving Christology progress:', err)
      );
    }
  }, [completedLessons, userId, setUserData]);

  const lessons = [
    { id: '01', title: 'Introduction to Christology', file: 'Unit_01_Introduction.md', icon: '✝️', duration: '15 min' },
    { id: '02', title: 'The Pre-Existence of Christ', file: 'Unit_02_Pre_Existence.md', icon: '⭐', duration: '20 min' },
    { id: '03', title: 'The Incarnation', file: 'Unit_03_Incarnation.md', icon: '👶', duration: '25 min' },
    { id: '04', title: 'The Two Natures of Christ', file: 'Unit_04_Two_Natures.md', icon: '⚖️', duration: '25 min' },
    { id: '05', title: 'The Ministry of Christ', file: 'Unit_05_Ministry.md', icon: '🙏', duration: '20 min' },
    { id: '06', title: 'The Atonement', file: 'Unit_06_Atonement.md', icon: '✟', duration: '30 min' },
    { id: '07', title: 'Resurrection and Ascension', file: 'Unit_07_Resurrection.md', icon: '👑', duration: '20 min' },
  ...
  const loadLessonContent = async (lesson) => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.PUBLIC_URL || ''}/SwordDrill_Associate_Christology/${lesson.file}`);
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-slate-400 mt-2">Doctrine of Christ & His Nature - Associate Level</p>
            </div>
            <button
              onClick={onCancel}
          ...
          <div className="mt-8 bg-amber-900/20 border-2 border-amber-500/30 rounded-xl p-6">
            ...
            <p className="text-slate-300 leading-relaxed">
              Examine the biblical doctrine of Christ, His divine and human natures, the incarnation, atonement,
              resurrection, and His eternal ministry as Prophet, Priest, and King.
            </p>
          </div>
        </div>
      </div>
