...
// Helper function to calculate points for a quiz
const calculateQuizPoints = (quizType, isCorrect, userLevel, quizTime, isPerfect = false, verseProgress = null, isPersonalVerse = false) => {
  // Handle incorrect answers with penalties
  if (!isCorrect) {
    // Use the level-specific penalty; default to Beginner to avoid NaN when config is missing
    const basePenalty =
      POINT_SYSTEM.PENALTIES.incorrectAnswer[userLevel] ??
      POINT_SYSTEM.PENALTIES.incorrectAnswer.Beginner ??
      -10;
    let penalty = Number.isFinite(basePenalty) ? basePenalty : -10;

    // Additional penalty for repeated mistakes on the same verse
    if (verseProgress && verseProgress.incorrectCount > 0) {
      const repeatedPenalty = Math.min(verseProgress.incorrectCount * 2, 20); // Max -20 for repeated mistakes
      penalty -= repeatedPenalty;
    }
  ...
  let points = Math.floor(basePoints * levelConfig.multiplier);
  ...
  // Time-based penalties using the new TIME_THRESHOLDS
  if (quizTime) {
    const thresholds = POINT_SYSTEM.TIME_THRESHOLDS[quizType] || POINT_SYSTEM.TIME_THRESHOLDS.default;
    let speedMultiplier = 1.0;

    // Penalize answers that are too fast (likely guessing/rushing)
    if (quizTime < thresholds.min) {
      speedMultiplier = 0.5; // 50% penalty for rushing
      points += POINT_SYSTEM.PENALTIES.tooFastAnswer; // Additional -10 point penalty
    }
    // Slightly penalize answers that are too slow
    else if (quizTime > thresholds.max) {
      speedMultiplier = 0.7; // 30% reduction for being too slow
    }
    // Reward ideal timing
    else if (quizTime >= thresholds.min && quizTime <= thresholds.ideal) {
      speedMultiplier = 1.0; // Full points
    }
    // Gradual reduction between ideal and max
    else if (quizTime > thresholds.ideal && quizTime <= thresholds.max) {
      const range = thresholds.max - thresholds.ideal;
      const position = quizTime - thresholds.ideal;
      speedMultiplier = 1.0 - (position / range) * 0.3; // Gradually reduce to 0.7
    }

    points = Math.floor(points * speedMultiplier);
  }

  // Cap points at 2 for personal memory verses
  if (isPersonalVerse && points > 2) {
    points = 2;
  }
