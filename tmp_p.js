...
export async function answerQuery(userMessage, context = {}) {
  ...
  if (routed.type === 'compare_translations' && parsed.valid && parsed.verse) {
    ...
    if (kjv || web || esv || asv) {
      ...
      const meta = { compare: { ref: parsed.normalized, kjv: kjv?.text || '', web: web?.text || '', esv: esv?.text || '', asv: asv?.text || '' } };
      return { answer: applyNeutrality(ans), citations: [{ ref: parsed.normalized, translation: 'KJV/WEB/ESV/ASV' }], meta };
    }
  }
